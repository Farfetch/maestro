...
from maestro_cli.services.maestro_api.run import (
    Run,
    RunApi,
    RunStatus,
)
...
def test_maestro_run_update(mocker):
    ...
    put_mock = mocker.patch(
        "maestro_cli.services.maestro_api.MaestroApiClient.put",
    )
...
def test_maestro_run_create(mocker):
    run_configuration_id = "1-2-3-4"
    data = {"run_configuration_id": run_configuration_id}

    post_mock = mocker.patch(
        "maestro_cli.services.maestro_api.MaestroApiClient.post",
    )

    RunApi.create(run_configuration_id)
